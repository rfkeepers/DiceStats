...
// this version rolls the stat dice twice and takes the better result
...
func initCounts(counts map[int]int, d1sides, d2sides int) {
	for i := 1; i <= d1sides; i++ {
		addCount(counts, i, -1, -1, d2sides)
	}
}

func addCount(counts map[int]int, d1, d2a, d2b, d2sides int) {
	if d2a > 0 {
		counts[-1] = counts[-1] + 1
		nsum := sum(d1, d2a, d2b)
		for _, b := range buckets {
	...
	for i := 1; i <= d2sides; i++ {
		for j := 1; j <= d2sides; j++ {
			addCount(counts, d1, i, j, d2sides)
		}
	}
}
